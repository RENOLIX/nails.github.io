...
function BreadcrumbList({ className, ...props }: React.ComponentProps<"ol">) {
  return <ol data-slot="breadcrumb-list" className={cn("text-muted-foreground flex flex-wrap items-center gap-1.5 text-sm sm:gap-2.5", className)} {...props} />;
}
...
function BreadcrumbLink({ className, ...props }: React.ComponentProps<"a"> & { asChild?: boolean }) {
  return <a data-slot="breadcrumb-link" className={cn("hover:text-foreground cursor-pointer transition-colors", className)} {...props} />;
}
